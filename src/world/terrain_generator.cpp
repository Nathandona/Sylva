...
std::pair<float, float> TerrainGenerator::generateBiomeData(float worldX, float worldZ, std::mt19937& rng) const {
    // Generate humidity
...
void TerrainGenerator::applyEnvironmentalEffects(Chunk* chunk, int x_local, int z_local, float height, float humidity, float temperature) const {
...
void TerrainGenerator::generateTerrain(Chunk* chunk) {
    ...
    for (int z_local = 0; z_local < CHUNK_SIZE; z_local++) {
        for (int x_local = 0; x_local < CHUNK_SIZE; x_local++) {
            ...
            // Generate biome data
            auto [humidity, temperature] = generateBiomeData(worldX, worldZ, rng);
            ...
            applyEnvironmentalEffects(chunk, x_local, z_local, finalHeight, humidity, temperature);
        }
    }
}

void TerrainGenerator::generateTrees(Chunk* chunk, std::mt19937& rng, std::uniform_int_distribution<int>& randPos) {
...
void TerrainGenerator::generateRocks(Chunk* chunk, std::mt19937& rng, std::uniform_int_distribution<int>& randPos) {
    std::uniform_real_distribution<float> rand01(0.0f, 1.0f);
...
void TerrainGenerator::generateVegetation(Chunk* chunk, std::mt19937& rng, std::uniform_int_distribution<int>& randPos) {
    std::uniform_real_distribution<float> rand01(0.0f, 1.0f);
...
void TerrainGenerator::generateDecorations(Chunk* chunk, std::mt19937& rng, std::uniform_int_distribution<int>& randPos) {
    std::uniform_real_distribution<float> rand01(0.0f, 1.0f);
...
void TerrainGenerator::generateFeatures(Chunk* chunk) {
    ...
    generateTrees(chunk, rng, randPos);
    generateRocks(chunk, rng, randPos);
    generateVegetation(chunk, rng, randPos);
    generateDecorations(chunk, rng, randPos);
}

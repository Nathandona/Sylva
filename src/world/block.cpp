...
void BlockData::initialize() {
    ...
    // AIR
    s_appearances[static_cast<size_t>(BlockType::AIR)] = BlockAppearance(
    ...
    s_appearances[static_cast<size_t>(BlockType::GRASS)] = BlockAppearance(
        glm::vec3(0.55f, 0.70f, 0.30f)
    );
    
    // DIRT - Richer, more textured brown
    s_appearances[static_cast<size_t>(BlockType::DIRT)] = BlockAppearance(
        glm::vec3(0.65f, 0.45f, 0.30f)
    );
    
    // STONE - More varied grey with slight blue tint for micro-detail
    s_appearances[static_cast<size_t>(BlockType::STONE)] = BlockAppearance(
        glm::vec3(0.70f, 0.68f, 0.65f)
    );
    
    // SAND - Softer, less saturated sand color
    s_appearances[static_cast<size_t>(BlockType::SAND)] = BlockAppearance(
        glm::vec3(0.88f, 0.81f, 0.64f)
    );
    
    // WOOD - Deeper, richer wood tone for contrast with terrain
    s_appearances[static_cast<size_t>(BlockType::WOOD)] = BlockAppearance(
        glm::vec3(0.60f, 0.40f, 0.20f)
    );
    
    // LEAVES - More varied green with slightly higher opacity
    s_appearances[static_cast<size_t>(BlockType::LEAVES)] = BlockAppearance(
        glm::vec3(0.40f, 0.65f, 0.28f), 0.85f
    );
    s_transparent[static_cast<size_t>(BlockType::LEAVES)] = true;
    
    // WATER - Deeper, more translucent blue
    s_appearances[static_cast<size_t>(BlockType::WATER)] = BlockAppearance(
        glm::vec3(0.12f, 0.45f, 0.72f), 0.7f
    );
